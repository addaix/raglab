...
def get_user_id_by_token(token) :
    user = SqlBaseKvReader(engine=engine_from_config, table_name="users", key_columns="token", value_columns=["id"])
    user_id = user[token]
    return user_id
...
class PromptPilote :
    ...
    def list(token:str, db:Session = get_db()) :
        ...
        apps = SqlBaseKvReader(engine=engine, table_name="app", key_columns="id", value_columns=["name"])
